...
class WLC_Lexware_API_Client {
    ...
    /**
     * Sucht nach einem bestehenden Kontakt in Lexware Office anhand der E-Mail-Adresse
     * 
     * @param string $email Die E-Mail-Adresse zum Suchen
     * @return array|null Der gefundene Kontakt oder null
     */
    public function find_contact_by_email($email) {
        ...
        $result = $this->request('GET', 'contacts?email=' . urlencode($email));
        
        if (is_wp_error($result)) {
            return null;
        }
        
        // Die API gibt ein Array mit 'content' zurück, das die Kontakte enthält
        if (isset($result['content']) && is_array($result['content']) && count($result['content']) > 0) {
            // Nimm den ersten passenden Kontakt
    ...
    public function sync_contact($order) {
        ...
        // Falls keine Kontakt-ID in der Bestellung gespeichert ist,
        // suche nach einem bestehenden Kontakt in Lexware Office anhand der E-Mail
        if (empty($existing_contact_id)) {
            $billing_email = $order->get_billing_email();
            $existing_contact = $this->find_contact_by_email($billing_email);
            
            if ($existing_contact && isset($existing_contact['id'])) {
                $existing_contact_id = $existing_contact['id'];
                // Speichere die gefundene Kontakt-ID für zukünftige Bestellungen
                $order->update_meta_data('_wlc_lexware_contact_id', $existing_contact_id);
                $order->save();
            }
        }
    ...
    public function create_invoice($order, $contact_id) {
        ...
        foreach ($voucher_items as $voucher) {
            $line_items[] = array(
                'type' => 'custom',
                'name' => 'Einlösung Gutschein',
                #'description' => 'Einlösung Gutschein',
                'quantity' => 1,  // Menge: 1
                'unitName' => __('Stück', 'lexware-connector-for-woocommerce'),
                'unitPrice' => array(
                    'currency' => $order->get_currency(),
                    'netAmount' => -$voucher['amount'],  // Preis: Negativ
                    'grossAmount' => -$voucher['amount'],  // Preis: Negativ
                    'taxRatePercentage' => 19  // 0% Steuersatz für Wertgutscheine
                )
            );
        }
